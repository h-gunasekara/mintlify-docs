---
title: 'Examples'
description: 'Interact with the Catalog API using different programming languages'
---
<Note>
  Replace `YOUR_API_KEY` with your actual API key in all examples.
</Note>

## Search API

The Search API allows you to find products using natural language queries and various filters.

### Basic Search

Search for products matching "cotton shirt":

<CodeGroup>

```javascript Javascript
// Search for "cotton shirt"
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/products", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({ query: "cotton shirt" })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Search for "cotton shirt"
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/products",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={"query": "cotton shirt"}
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/products \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "query": "cotton shirt"
  }'
```

</CodeGroup>

### Using Filters

Filter products by vendor, availability, and price range:

<CodeGroup>

```javascript Javascript
// Filter by vendor, availability, price
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/products", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({
      filters: {
        vendor: "EcoWear",
        is_available: true,
        price_min: 20.00,
        price_max: 50.00
      }
    })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Filter by vendor, availability, price
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/products",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={
        "filters": {
            "vendor": "EcoWear",
            "is_available": True,
            "price_min": 20.00,
            "price_max": 50.00
        }
    }
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/products \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "filters": {
      "vendor": "EcoWear",
      "is_available": true,
      "price_min": 20.00,
      "price_max": 50.00
    }
  }'
```

</CodeGroup>

### Filtering by Attributes

Filter products by color and fabric:

<CodeGroup>

```javascript Javascript
// Filter by attributes (color, fabric)
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/products", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({
      filters: {
        attributes: {
          color: "Green",
          fabric: {
            cotton: true
          }
        }
      }
    })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Filter by attributes (color, fabric)
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/products",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={
        "filters": {
            "attributes": {
                "color": "Green",
                "fabric": {
                    "cotton": True
                }
            }
        }
    }
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/products \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "filters": {
      "attributes": {
        "color": "Green",
        "fabric": {
          "cotton": true
        }
      }
    }
  }'
```

</CodeGroup>

### Pagination

Get the second page of search results for "shoes":

<CodeGroup>

```javascript Javascript
// Search with pagination (page 2)
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/products", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({
      query: "shoes",
      page: 2
    })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Search with pagination (page 2)
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/products",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={
        "query": "shoes",
        "page": 2
    }
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/products \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "query": "shoes",
    "page": 2
  }'
```

</CodeGroup>

## Feed API

The Catalog Feed API allows you to get a paginated feed of products from a specific vendor.

### Basic Feed Request

Get the first page of products from a specific vendor:

<CodeGroup>

```javascript Javascript
// Get first page of products from Saintlaurent
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/feed", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({
      vendor: "Saintlaurent",
      page: 1
    })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Get first page of products from Saintlaurent
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/feed",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={
        "vendor": "Saintlaurent",
        "page": 1
    }
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/feed \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "vendor": "Saintlaurent",
    "page": 1
  }'
```

</CodeGroup>

### Feed Pagination

Get the second page of products from a vendor:

<CodeGroup>

```javascript Javascript
// Get second page of products from Jennikayne
(async () => {
  const API_KEY = "YOUR_API_KEY"; // Replace with your actual key
  
  const response = await fetch("https://catalogai.vercel.app/api/feed", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': API_KEY
    },
    body: JSON.stringify({
      vendor: "Jennikayne",
      page: 2
    })
  });

  const data = await response.json();
  console.log(data);
})();
```

```python Python
import requests

# Get second page of products from Jennikayne
API_KEY = "YOUR_API_KEY"  # Replace with your actual key

response = requests.post(
    "https://catalogai.vercel.app/api/feed",
    headers={
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    },
    json={
        "vendor": "Jennikayne",
        "page": 2
    }
)

data = response.json()
print(data)
```

```bash cURL
curl -X POST https://catalogai.vercel.app/api/feed \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -d '{
    "vendor": "Jennikayne",
    "page": 2
  }'
```

</CodeGroup>
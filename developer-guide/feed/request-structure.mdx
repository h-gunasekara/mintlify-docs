...
1. **Remove Spaces**: Multi-word brand names are typically concatenated
   - "Jenni Kayne" → `"Jennikayne"`
   - "Saint Laurent" → `"Saintlaurent"`

2. **Remove Special Characters**: Accents and special characters are usually removed
   - "Sézane" → `"Sezane"`

3. **Capitalize First Letter**: Follow title case formatting
   - "ganni" → `"Ganni"`
   - "balenciaga" → `"Balenciaga"`

4. **Preserve Brand Identity**: Some brands maintain specific formatting
   - "COS" remains `"COS"`
   - "A.P.C." becomes `"Apc"`
...
1. **Check the Domain**: Use the main part of the website domain
   - `https://www.ganni.com/` → `"Ganni"`
   - `https://shop.mango.com/` → `"Mango"`

2. **Test Variations**: Try different formatting approaches
...
- **`vendor`**: 
  ...
  - Recommended to use capitalized format (e.g., "Ganni" not "ganni")
...
// Example usage
const vendorVariations = ['Ganni', 'ganni', 'GANNI'];
